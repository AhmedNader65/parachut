...
public class COMMONProducStoresDataSource extends PageKeyedDataSource<Long, Datum> {
    public static int PAGE_SIZE = 8;
    public static long FIRST_PAGE = 1;
GlobalPrefrencies globalPrefrencies;
    String id_;
    public COMMONProducStoresDataSource(String id_, Context context) {
    ...
    public void loadInitial(@NonNull LoadInitialParams<Long> params, @NonNull final LoadInitialCallback<Long, Datum> callback) {
        ...
        RetroWeb.getClient().create(ServiceApi.class).onGetMostCOMMONProductStores(id_,globalPrefrencies.getLanguage(),FIRST_PAGE).enqueue(new Callback<AllProductCategories>() {
            @Override
    ...
    public void loadBefore(@NonNull final LoadParams<Long> params, @NonNull final LoadCallback<Long, Datum> callback) {
        ...
        RetroWeb.getClient().create(ServiceApi.class).onGetMostCOMMONProductStores(id_,globalPrefrencies.getLanguage(),params.key).enqueue(new Callback<AllProductCategories>() {
            @Override
            public void onResponse(Call<AllProductCategories> call, Response<AllProductCategories> response) {
                AllProductCategories body = response.body();
                if (body != null) {
