...
public class MAXProductStoresDataSource extends PageKeyedDataSource<Long, Datum> {
    ...
    String id_;
    public MAXProductStoresDataSource(String id_) {
        this.id_=id_;
    }
    ...
    public void loadInitial(@NonNull LoadInitialParams<Long> params, @NonNull final LoadInitialCallback<Long, Datum> callback) {
        ...
        RetroWeb.getClient().create(ServiceApi.class).onGetMAXAllProductStores(id_,FIRST_PAGE).enqueue(new Callback<AllProductCategories>() {
            @Override
    ...
    public void loadBefore(@NonNull final LoadParams<Long> params, @NonNull final LoadCallback<Long, Datum> callback) {
        ...
        RetroWeb.getClient().create(ServiceApi.class).onGetMAXAllProductStores(id_,params.key).enqueue(new Callback<AllProductCategories>() {
            @Override
            public void onResponse(Call<AllProductCategories> call, Response<AllProductCategories> response) {
                AllProductCategories body = response.body();
                if (body != null) {
