...
public class MAXOffersDataSource extends PageKeyedDataSource<Long, Datum> {
    public static int PAGE_SIZE = 8;
    public static long FIRST_PAGE = 1;

    GlobalPrefrencies globalPrefrencies;
    public MAXOffersDataSource(Context context) {
    ...
    public void loadInitial(@NonNull LoadInitialParams<Long> params, @NonNull final LoadInitialCallback<Long, Datum> callback) {
        ...
        RetroWeb.getClient().create(ServiceApi.class).onGetMAXAllOffersModel(globalPrefrencies.getLanguage(),FIRST_PAGE).enqueue(new Callback<AllOffersModel>() {
            @Override
    ...
    public void loadBefore(@NonNull final LoadParams<Long> params, @NonNull final LoadCallback<Long, Datum> callback) {
        ...
        RetroWeb.getClient().create(ServiceApi.class).onGetMAXAllOffersModel(globalPrefrencies.getLanguage(),params.key).enqueue(new Callback<AllOffersModel>() {
            @Override
            public void onResponse(Call<AllOffersModel> call, Response<AllOffersModel> response) {
                AllOffersModel body = response.body();
                if (body != null) {
